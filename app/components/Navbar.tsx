  ...
  return (
    <>
      ...
      <div
        ...
        style={{ background: '#012d329d', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}
        aria-hidden={!menuOpen}
      >
        <div className="flex items-center justify-between px-5 h-16 flex-shrink-0" style={{ background: '#012D32' }}>
          <Link href="/" onClick={() => setMenuOpen(false)}>
            <Image src="/Group.png" alt="ChangPay" width={110} height={40} />
          </Link>
          <button onClick={() => setMenuOpen(false)} className="w-10 h-10 flex items-center justify-center bg-transparent border-none cursor-pointer" aria-label="Close menu">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-0">
          {NAV_LINKS.map(({ label, href }, i) => {
            const isActive = href === '/#how-it-works' ? false : pathname === href;
            return (
              <Link
                key={label}
                href={href}
                ...
                style={{
                  ...
                  animation: menuOpen ? `mobileNavIn .35s cubic-bezier(.16,1,.3,1) ${i * 55 + 80}ms forwards` : 'none',
                }}
              >
                {label}
              </Link>
          ...
          <Link
            ...
            style={{
              opacity: 0,
              transform: 'translateY(16px)',
              animation: menuOpen ? `mobileNavIn .35s cubic-bezier(.16,1,.3,1) ${NAV_LINKS.length * 55 + 80}ms forwards` : 'none',
            }}
          >
            Create an account
          </Link>
