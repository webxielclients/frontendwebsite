...
document.getElementById("how-it-works")?.scrollIntoView({ behavior: "smooth" });
  ...
  return (
    <>
      ...
      <header
        className={`sticky top-0 z-50 transition-all duration-300 border-b border-gray-100 ${scrolled ? "bg-white/95 backdrop-blur-md shadow-sm" : "bg-white"}`}
      >
        <nav className="nav-font max-w-[1200px] mx-auto px-5 md:px-12 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            {NAV_LINKS.map(({ label, href }) => {
              const isActive =
                href === "/#how-it-works" ? false : pathname === href;
              return (
                <Link
                  key={label}
                  href={href}
                ...
                >
                  {label}
                  <span
                    className={`absolute -bottom-1 left-0 h-0.5 bg-[#16a34a] rounded-full transition-all duration-300 ${isActive ? "w-full" : "w-0 group-hover:w-full"}`}
                  />
                </Link>
              );
            })}
